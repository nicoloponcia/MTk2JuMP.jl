...
function set_gDyn!(OCPI::OCPInterface_;
    f_scale::Union{Float64,Vector{Float64}, Matrix{Float64}, Vector{JuMP.AffExpr}, Vector{JuMP.QuadExpr},Vector{JuMP.NonlinearExpr}, Matrix{JuMP.AffExpr}, Matrix{JuMP.QuadExpr}, Matrix{JuMP.NonlinearExpr}}=1.0)

    if OCPI.settings.int_method == :EE
        f_scale = f_scale isa Number ? fill(f_scale, OCPI.settings.N-1) : f_scale
        EE_dyn_constraints!(OCPI; f_scale=f_scale)
    elseif OCPI.settings.int_method == :Coll
        f_scale = f_scale isa Number ? fill(f_scale, OCPI.settings.N-1, OCPI.settings.Coll_set.order+1) : f_scale
